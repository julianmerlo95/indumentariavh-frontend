import React from 'react';
import Modal from "react-bootstrap/Modal";
import Button from "@mui/material/Button";

function ClientSelected(props) {
    return (
        <div className="invoice-form-header-description-client">
            <div>
                <h4>CLIENTE SELECCIONADO</h4>
                <table className="current-account-client-table">
                    <thead>
                    <tr className="current-account-client-table-head">
                        <th scope="col">Id</th>
                        <th scope="col">Nombre</th>
                        <th scope="col">Apellido</th>
                        <th scope="col">Dni</th>
                        <th scope="col">Deuda</th>
                    </tr>
                    </thead>
                    <tbody className="current-account-client-table-body">
                    <th scope="row">{props.clientSelected.idClient}</th>
                    <th scope="row">{props.clientSelected.name}</th>
                    <th scope="row">{props.clientSelected.lastName}</th>
                    <th scope="row">{props.clientSelected.dni}</th>
                    <th scope="row">$<span className="moneyDebt-client-span">{new Intl.NumberFormat("es-CL").format(props.clientSelected.moneyDebt)}</span></th>
                    </tbody>
                </table>
            </div>
            <div> {/* Header invoice - Show client model */}
                <Modal className="invoice-form-header-description-seller-modal" size="lg" show={props.showClient}
                       onHide={props.clientCloseHandler}>
                    <Modal.Header closeButton>
                        <Modal.Title>Listado de clientes</Modal.Title>
                    </Modal.Header>
                    <Modal.Body className="invoice-form-header-description-seller-modal-body">
                        <div>
                            <div className="table table-body table-responsive">
                                <table className="table table-body">
                                    <thead>
                                    <tr>
                                        <th scope="col">Id</th>
                                        <th scope="col">Nombre</th>
                                        <th scope="col">Apellido</th>
                                        <th scope="col">Dni</th>
                                        <th scope="col">Deuda</th>
                                    </tr>
                                    </thead>
                                    <tbody>
                                    {props.clients && props.clients.length > 0 ? props.clients.map((client, index) => {
                                        return (
                                            <tr key={index} className="table table-body-data">
                                                <th scope="row">{client.idClient}</th>
                                                <th scope="row">{client.name}</th>
                                                <th scope="row">{client.lastName}</th>
                                                <th scope="row">{client.dni}</th>
                                                <th scope="row">$<span className="moneyDebt-client-span">{new Intl.NumberFormat("es-CL").format(Number(client.moneyDebt))}</span></th>
                                                <Button className="client-high-button-select" variant="contained" size="small" color="success"
                                                        onClick={() => props.clientSelectedHandler(client)}>Elegir
                                                </Button>
                                            </tr>
                                        )
                                    }) : []
                                    }
                                    </tbody>
                                </table>
                            </div>
                        </div>
                    </Modal.Body>
                    <Modal.Footer>
                        <Button className="client-high-button-error" variant="contained" size="large" color="error" onClick={props.clientCloseHandler}>
                            Cerrar
                        </Button>
                    </Modal.Footer>
                </Modal>
            </div>
        </div>
    )
}

export default ClientSelected;